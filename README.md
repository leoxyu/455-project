...
Alex<br>
I initially completed the UI for the playlists page, setting up components and styles for a Spotify-inspired grid layout. Then, I created the redux store for playlists and songs using redux toolkit, eventually hooking it up to async thunks that called a REST API for playlists and songs. This REST API was initially created by another team member, but I updated it to use mongo database queries instead of in-memory storage. I implemented cursor pagination, filtering, and sorting for some of the endpoints. In the frontend, I fine-tuned an infinite scroll component library to work with the playlist page and with the cursor pagination. I realized schema inconsistency would be an issue, so I created a schema validator as a source of truth for the playlists collection. When mongo was all set up, I created an endpoint to batch import Spotify playlists into the database that would report all successful and failed imports. Finally, I noticed that the frontend was getting extremely slow due to the quantity of data (thousands of songs) being fetched and manipulated in redux, so I made some optimizations like lazy loading songs and eliminating API calls.
